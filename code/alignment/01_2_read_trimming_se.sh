...
read_trimming () {
    SAMPLE=`echo $1`
    LANE=`ls $RAW/$SAMPLE`
    R1="$RAW/$SAMPLE/$LANE/${LANE}.fastq.gz"
    ...
    
    {
        ...
        CMD1="trim_galore --cores 4 -q 30 --length 20 --fastqc_args \"--nogroup\" --output_dir $OUTPUT $R1"
        echo $CMD1
        eval $CMD1
        ...
        CMD2='for i in `find $OUTPUT -name "*_trimmed.fq.gz"`; do mv "$i" "${i/_trimmed.fq.gz/.fastq.gz}"; done'
        echo "$CMD2"
        eval "$CMD2"
